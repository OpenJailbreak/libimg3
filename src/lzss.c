...
uint32_t lzss_adler32(uint8_t *buf, int32_t len)
{
...
int
lzss_decompress(uint8_t *dst, uint8_t *src, uint32_t srclen)
{
    /* ring buffer of size N, with extra F-1 bytes to aid string comparison */
    uint8_t text_buf[N + F - 1];
...
uint8_t *
lzss_compress(uint8_t *dst, uint32_t dstlen, uint8_t *src, uint32_t srcLen)
{
